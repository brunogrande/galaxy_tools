...
if (is.null(opt$cnv.png) | is.null(opt$cnv.txt) | is.null(opt$exon.lrr.txt) |
    ...
    cat("You must specify names for all output files\n", file=stderr())
    quit(status=1)
}
...
file.rename(paste0(stem, ".cnv.png"), opt$cnv.png)
file.rename(paste0(stem, ".cnv.txt"), opt$cnv.txt)
file.rename(paste0(stem, ".exon.lrr.txt"), opt$exon.lrr.txt)
file.rename(paste0(stem, ".segment.copynumber.txt"), opt$segment.copynumber.txt)
file.rename(paste0(stem, ".segment.lrr.txt"), opt$segment.lrr.txt)
